...
type arrayOrMapResult struct {
	a  []JsonRes
	ai []interface{}
	o  map[string]JsonRes
	oi map[string]interface{}
	vc byte
}

func (r JsonRes) arrayOrMap(vc byte, valueize bool) (ar arrayOrMapResult) {
	var j = r.Raw
	var i int
	var value JsonRes
	var count int
...
func ParseBytes(json []byte) JsonRes {
	return Parse(Bytes2String(json))
}
...
func parseQuery(query string) (
	...
	if j > 0 {
		path = TrimSpace(query[2:j])
		value = TrimSpace(query[j:i])
		remain = query[i+1:]
		var opsz int
		switch {
		case len(value) == 1:
		...
		op = value[:opsz]
		value = TrimSpace(value[opsz:])
	} else {
		path = TrimSpace(query[2:i])
		remain = query[i+1:]
	}
...
func queryMatches(rp *arrayPathResult, value JsonRes) bool {
	rpv := rp.query.value
...
func parseArray(c *parseContext, i int, path string) (int, bool) {
	...
	procQuery := func(qval JsonRes) bool {
		...
		for ; ; i++ {
			...
			case '{':
				if pmatch && !hit {
					...
					val, i = parseSquash(c.json, i)
					if rp.query.on {
						if procQuery(JsonRes{Raw: val, Type: JSON}) {
							return i, true
						}
					} else if hit {
						if rp.alogok {
			...
			case ']':
				if rp.arrch && rp.part == "#" {
					if rp.alogok {
						...
						for j, k := 0, 0; j < len(alog); j++ {
							...
							if ok {
								...
								if res.Exists() {
									...
									jsons = append(jsons, String2Bytes(raw)...)
									k++
								}
							}
						}
						jsons = append(jsons, ']')
						c.value.Type = JSON
						c.value.Raw = Bytes2String(jsons)
						return i + 1, true
					}
					if rp.alogok {
						break
				...
				if len(multires) > 0 && !c.value.Exists() {
					c.value = JsonRes{
						Raw:  Bytes2String(append(multires, ']')),
						Type: JSON,
					}
				}
				return i + 1, false
...
func appendJSONString(dst []byte, s string) []byte {
	for i := 0; i < len(s); i++ {
		if s[i] < ' ' || s[i] == '\\' || s[i] == '"' || s[i] > 126 {
			d, _ := json.Marshal(s)
			return append(dst, Bytes2String(d)...)
		}
	}
	dst = append(dst, '"')
	dst = append(dst, s...)
...
func GetBytes(json []byte, path string) JsonRes {
	return Get(Bytes2String(json), path)
}
...
func parseAny(json string, i int, hit bool) (int, JsonRes, bool) {
	var res JsonRes
	var val string
	for ; i < len(json); i++ {
		if json[i] == '{' || json[i] == '[' {
...
func assign(jsval JsonRes, val reflect.Value) {
	if jsval.Type == Null {
		return
	}
	switch val.Kind() {
	...
	case reflect.Struct:
		...
		jsval.ForEach(func(key, value JsonRes) bool {
	...
	case reflect.Array:
		i, n := 0, val.Len()
		jsval.ForEach(func(_, value JsonRes) bool {
			if i == n {
				return false
			}
...
func Valid(json string) (ok bool) {
	_, ok = validPayload(String2Bytes(json), 0)
	return
}
...
func modifierPretty(json, arg string) string {
	if len(arg) > 0 {
		opts := *DefOptions
		Parse(arg).ForEach(func(key, value JsonRes) bool {
			switch key.String() {
			case "sortKeys":
		...
		return Bytes2String(FormatOptions(String2Bytes(json), &opts))
	}
	return Bytes2String(Format(String2Bytes(json)))
}

func modifierUgly(json, _ string) string {
	return Bytes2String(Ugly(String2Bytes(json)))
}
...
func modifierReverse(json, _ string) string {
	...
	if res.IsArray() {
		var values []JsonRes
		res.ForEach(func(_, value JsonRes) bool {
	...
	if res.IsObject() {
		var keyValues []JsonRes
		res.ForEach(func(key, value JsonRes) bool {
